...
import { getDBCredentialsFromLocation } from './locate';

export const getAllUsersStatistics = (user: User, container: Container, done: any) => {
    const creds = getDBCredentialsFromLocation(container.location);
    ...
    .then(() => {
        ...
        const activeUsersPromise = client.query(`
            ...
            AND 
                s."loggedInDate" >= '${statsLimitDate.toISOString()}';
        `);
        console.log(statsLimitDate.toISOString());
